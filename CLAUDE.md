...
- 밸런스 필수 게이트 PASS (20시드): 1–3 오토100%, 8–10 오토0%. 후반 배율 조정은 아래 M3 최종 결과 참조.

### M3 최종 밸런스 — 2026-09-06
- 영웅/피해 수치 변경 없이 스테이지8 배율1.75, 9–10 배율1.55로 조정. 초기 선형배율에서는 수동 비교 정책도 9–10 승리0이라 최종 보스를 깰 수 있게 조정했다.
- 20시드: 오토 1–3=100/100/100%, 8–10=10/35/0%. 전술 정책 8–10=95/55/40%. 오토/궁없음/전술 600판 보고서 생성.
- 전체 단위/회귀29개 PASS, 마지막 스테이지 전술정책 승리와 오토패배를 동일시드2로 검증.

### M4 — 2026-09-06
- 사용자 제공 아트 10종/초상 10종을 복사해 단일 2048² 아틀라스로 통합. 키아트 절차 애니메이션, 3레이어 패럴랙스, 자체 합성 SFX 6종 완료. 도형 플레이스홀더 0개.
- 재사용 아트와 최종 캐릭터 외형의 차이는 PROVENANCE.md와 gen-prompts.md에 명시. 새 원본으로 교체 가능하며 기존 아트 원본은 수정하지 않았다.
- 스틸로 HUD 발 가림 수정, 조준 범위, 실제 INTERRUPT 확인.

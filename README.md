...
- 초기 선형 적 배율은 후반 두 보스 조합을 사실상 클리어 불가로 만들었습니다. 브리프의 밸런스 조정 지시에 따라 8스테이지는 1.75, 9~10은 1.55로 조정했습니다. 20시드에서 오토는 10%/35%/0%, 시전 차단·회복 타이밍을 고르는 비교 정책은 95%/55%/40%입니다.
...
터치 이벤트를 사용한 방향 조준도 검증했습니다. 20유닛·CPU 6배 제한 Chrome 성능 측정은 `tests/performance-report.json`에 기록합니다.
